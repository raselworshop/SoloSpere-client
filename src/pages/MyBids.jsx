...
const MyBids = () => {
  ...
  const fetchAllBids = useCallback(async () => {
    try {
      const { data } = await axios.get(`${import.meta.env.VITE_API_URL}/client/my-bids/${user?.email}`);
      setBids(data);
      console.log(data)
    } catch (error) {
            ...
            <div className='overflow-hidden border border-gray-200  md:rounded-lg'>
              <table className='min-w-full divide-y divide-gray-200'>
                ...
                <tbody className='bg-white divide-y divide-gray-200 '>
                  {
                    bids.map(bid => <BidTable key={bid._id} bid={bid} handleStatusChange={handleStatusChange} />)
                  }
                </tbody>
              </table>
            </div>
